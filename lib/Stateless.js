...
api.Groups.index = function(at, callback) {
    getRequest(at, "/groups?omit=memberships", { doParse: true }, callback);
}
...
// Index of DM chats a user has
// Returns a paginated list of direct message chats, or conversations, sorted by updated_at descending.
//  opts.page     - Page number integer
//  opts.per_page - Entrys per page integer
api.Chats.index = function(at, opts, callback) {
    getRequest(at, "/chats", { opts: opts, doParse: true }, callback);
}
...
// List the latest 10 polls.
// If more polls exist, "continuation_token" will be returned
// Calling this function with continuation_token passed in will get the next 10 polls.
//  group_id [required] - string, the id of the group
//  continuation_token  - string, continuation_token as passed from the previous Polls.index call
api.Polls.index = function(at, group_id, continuation_token, callback) {
    let config = {
        doParse: true
    };

    if (continuation_token != null) {
        config.opts = { continuation_token: continuation_token };
    }

    getRequest(at, "/poll/" + group_id, config, callback);
}

// Create a poll.
//  group_id [required] - string, the id of the group
//  subject [required]  - string, the subject/title of the poll
//  options [required]  - string array, the poll options (length must be between 2 and 10)
//  expiration          - number, the expiration date of the poll as an epoch timestamp (number of seconds since 1 Jan 1970). Defaults to 24 hours from creation
api.Polls.create = function(at, group_id, opts, callback) {
    const { subject, options, expiration } = opts;

    if (!subject || !options) {
        callback({ error: "Must include a subject and options parameters" });
    } else if (options.length < 2 || options.length > 10) {
        callback({ error: "Options array must be between 2 and 10 options" });
    } else {
        const defaultExpirationDate = Math.floor(new Date() / 1000) + 24 * 60 * 60; //epoch time in seconds + num seconds in 24 hours
        ...
        postRequest(at, "/poll/" + group_id, { body: poll, responseCode: 200 }, callback);
    }
}
